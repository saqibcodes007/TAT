...
# Define a temporary folder for processed files. It will be /tmp on Azure.
...
# --- ALL NORMALIZATION AND HELPER FUNCTIONS FROM THE ORIGINAL FILE ARE KEPT IDENTICAL ---
# [Note: All functions like normalize_practice_name, normalize_provider_name, create_api_client_adapted, etc., are included here but hidden for brevity. The code you copy will contain them.]
...
def create_api_client_adapted(wsdl_url):
    ...
    try:
        ...
        session_req.timeout = 60
        transport = Transport(session=session_req, timeout=60)
...
# --- ALL OTHER HELPER AND PROCESSING FUNCTIONS ---
# Functions like validate_spreadsheet_adapted, run_all_phases_processing_adapted, etc.
# remain the same as the original file. They are included in the full code block.
...
def create_service_line_payload_phase3(client_obj, service_line_data_dict, encounter_start_dt_api_str, encounter_end_dt_api_str, row_identifier_log, actual_headers_map_param):
    ...
    def clean_value_p3(val, is_modifier=False, is_diag=False):
...
# --- NEW, SIMPLIFIED Flask Route Handlers ---

@app.route('/api/process', methods=['POST'])
def process_file_route_synchronous():
    ...
    display_message("info", f"Synchronous processing started for file: {file.filename}")

    try:
        # --- Create API Client and Header ---
        credentials = {
            "CustomerKey": customer_key, "User": username,
            "Password": escape_xml_special_chars(password)
        }
        ...
        if not tebra_header:
            return jsonify({"error": "Failed to build Tebra API request header."}), 500
        ...
        # Get the base of the original filename (e.g., "abc" from "abc.xlsx")
        base_input_filename = os.path.splitext(file.filename)[0]

        # Create the user-facing filename you requested
        user_facing_download_name = f"{base_input_filename} TebraAuto Results.xlsx"

        # Create a unique internal filename for the server to avoid conflicts
        server_filename = f"processed_{uuid.uuid4().hex}.xlsx"
        output_filepath = os.path.join(UPLOAD_FOLDER, server_filename)
        
        # This formatting logic is from the original background_task_processor
        ...
        with pd.ExcelWriter(output_filepath, engine='xlsxwriter', date_format='mm-dd-yyyy', datetime_format='mm-dd-yyyy') as writer:
        ...
        display_message("info", f"✅ Processed data saved to '{output_filepath}'")

        # --- Return final response ---
        # Add the server filename (for the URL) and the desired download name to the response
        summary_stats["server_filename"] = server_filename
        summary_stats["download_filename"] = user_facing_download_name
        return jsonify(summary_stats), 200

    except Exception as e:
        display_message("error", f"An error occurred during synchronous processing: {e}")
...
@app.route('/api/download_processed_file/<filename_on_server>', methods=['GET'])
def download_processed_file(filename_on_server):
    ...
    # Get the desired user-facing filename from the query parameter
    # Provide a fallback just in case it's missing
    desired_download_name = request.args.get('download_name', 'TebraAuto_Results.xlsx')
    ...
    try:
        ...
        return send_file(file_to_download_path, as_attachment=True, download_name=desired_download_name)
    except Exception as e:
...
# This makes the app runnable directly for local testing
